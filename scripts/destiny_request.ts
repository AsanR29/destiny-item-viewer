...
import {destiny_data as DD} from "./destiny_data.js";
import {DestinyPlayer, PlayerSession} from "./destiny_player.js";
import { NextFunction, urlencoded } from 'express';

export class DestinyRequest {
    session_id : string;
    data : any;
    run_info : any;
    static BUNGIE_ENDPOINTS : {[key:string]:string} = { 
    ...
    static async fetchManifest() {
        ...
        let manifest_opp = new DestinyRequest('',{});
        if(!r_url){ return; }
        ...
        for(let i = 0; i < endpoint_names.length; i++) {
            let name = endpoint_names[i];
            if(name){
                this.BUNGIE_ENDPOINTS[name] = manifest_data[name]; }
        ...
        let lore_opp = new DestinyRequest('', {"target":DD.lore_directory});
        lore_opp.create(
        ...
        let item_opp = new DestinyRequest('', {"target":DD.item_definitions});
        item_opp.create(
        ...
        let sandbox_opp = new DestinyRequest('', {"target":DD.perk_definitions});
            sandbox_opp.create(
        ...
        let socket_opp = new DestinyRequest('', {"target":DD.sockettype_definitions});
        socket_opp.create(
        ...
        let plugset_opp = new DestinyRequest('', {"target":DD.plugset_definitions});
        plugset_opp.create(
        ...
        let damagetype_opp = new DestinyRequest('', {"target":DD.damagetype_definitions});
    ...
    constructor(SESSION_ID : string, data : any) { 
        this.session_id = SESSION_ID;
        //this.API_KEY = API_KEY;
        this.data = data;
        this.run_info = {};
    }
    async findUser(form : any) {
    ...
    async authenticate_1(res : any, session_id : string) {
    ...
    async authenticate_2(query : any) {
        ...
        let result_2 = await this.checkBungieNetId(bungieNetId);
        if(result_2) {
    ...
    async checkBungieNetId(membershipId : string) {
    ...
    async getItems(data : any) {
        ...
        for(let i = 0; i < 3; i++)
        {
            let an_id = ids[i];
            if(an_id){
                player.chara_ids[i] = an_id;
                player.chara_metadata = data.Response["characters"].data[an_id];
            }
        }
    ...
    async getItemInstance(itemInstanceId : string) {
        let player = DD.player_directory[this.session_id];
    ...
    async create(url : string, body_params : any, next_function : NextFunction | boolean, method_type : string, alt_headers : any=false) {
        try {
            // Create the URL
            var r_url = url;
            ...
            if(next_function){ console.log("next function!"); 
                if(!(typeof next_function === 'boolean')){ await next_function(data); }
            }
            return data;
        } catch (error) {
            // Handle any errors
    ...
    async formCreate(url : string, body : any, next_function : NextFunction | boolean, method_type : string, headers : any) {
        var r_url = url;
        let response = null;
        if(method_type == "POST")
        {
    ...
    async genericSaveDefinitions(def_data : any, the_dict:any=false) {
...
function CreateError(code : number,r_url : string) { 
    let the_error = errors[code] as request_error;
    return the_error.clone(r_url);
}   // to look pretty
class request_error {
    is_error : boolean;
    code : number;
    ...
    constructor(code : number,next_function : NextFunction){
        this.is_error = true;
        this.code = code;
        this.r_url = '';
...
const errors : {[key:number]:request_error} = {
    503: new request_error(503,error_msg_page)  //idk
}
function error_msg_page(res : any){
